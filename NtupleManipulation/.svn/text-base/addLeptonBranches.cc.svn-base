...
void addLeptonBranches( string pathToNtuples = "/afs/cern.ch/user/m/mazhang/eos/atlas/user/m/mazhang/Data/Ntuples/v00-14" ){
...
void addWeightsToSample(string sampleID, string pathToNtuples) {
    ...
    string  filename2      = Form("%s/New/%s.root",pathToNtuples.c_str(),sampleID.c_str()); 
    ...
    TBranch *b_weight      = T2->Branch("weight1fb"        , &weight1fb         ,   "weight1fb/F"       );
    TBranch *b_trigw       = T2->Branch("trigweight"       , &trigweight        ,   "trigweight/F"      );
    TBranch *b_lepw        = T2->Branch("lepweight"        , &lepweight         ,   "lepweight/F"       );
    TBranch *b_nEvents     = T2->Branch("nGenEvents"       , &nGenEvents        ,   "nGenEvents/F"      );

    TBranch *b_weight_vec  = T2->Branch("weight1fb_vec"    , &weight1fb_vec     ,   "weight1fb_vec/F"   );
    TBranch *b_trigwv      = T2->Branch("trigweight_vec"   , &trigweight_vec    ,   "trigweight_vec/F"  );
    TBranch *b_lepwv       = T2->Branch("lepweight_vec"    , &lepweight_vec     ,   "lepweight_vec/F"   );
    TBranch *b_chan        = T2->Branch("channel_vec"      , &channel_vec       ,   "channel_vec/I"     );

    TBranch *b_pt1         = T2->Branch("lep1_pT_vec"      , &lep1_pT_vec	     ,   "lep1_pT_vec/F"     );	   
    TBranch *b_eta1        = T2->Branch("lep1_eta_vec"     , &lep1_eta_vec	     ,   "lep1_eta_vec/F"    );	   
    TBranch *b_phi1        = T2->Branch("lep1_phi_vec"     , &lep1_phi_vec	     ,   "lep1_phi_vec/F"    );	   
    TBranch *b_weight1     = T2->Branch("lep1_weight_vec"  , &lep1_weight_vec   ,   "lep1_weight_vec/F" );	   
    TBranch *b_pdg1        = T2->Branch("lep1_pdgId_vec"   , &lep1_pdgId_vec    ,   "lep1_pdgId_vec/I"  );	   

    TBranch *b_pt2         = T2->Branch("lep2_pT_vec"      , &lep2_pT_vec	     ,   "lep2_pT_vec/F"     );	   
    TBranch *b_eta2        = T2->Branch("lep2_eta_vec"     , &lep2_eta_vec	     ,   "lep2_eta_vec/F"    );	   
    TBranch *b_phi2        = T2->Branch("lep2_phi_vec"     , &lep2_phi_vec	     ,   "lep2_phi_vec/F"    );	   
    TBranch *b_weight2     = T2->Branch("lep2_weight_vec"  , &lep2_weight_vec   ,   "lep2_weight_vec/F" );	   
    TBranch *b_pdg2        = T2->Branch("lep2_pdgId_vec"   , &lep2_pdgId_vec    ,   "lep2_pdgId_vec/I"  );	   

    TBranch *b_mll         = T2->Branch("mll_vec"          , &mll_vec	     ,   "mll_vec/F"         );	   
    TBranch *b_zpt         = T2->Branch("Z_pt_vec"         , &Z_pt_vec	     ,   "Z_pt_vec/F"        );	   
    TBranch *b_zphi        = T2->Branch("Z_phi_vec"        , &Z_phi_vec	     ,   "Z_phi_vec/F"       );	   
    TBranch *b_dphi        = T2->Branch("dphill_vec"       , &dphill_vec        ,   "dphill_vec/F"      );	   
    ...
    for (Long64_t i=0;i<nentries;i++) {
        ...
        for( int iL = 0 ; iL < lep_n ; iL++ ){
            ...
            for( int jL = iL+1 ; jL < lep_n ; jL++ ){
                ...
                if (lep_pdgId->at(iL) * lep_pdgId->at(jL) > 0) // same sign
                    continue;
        ...
        if( imax < 0 || jmax < 0 || maxpt < 0 ){
            //cout << "No suitable lepton pair found" << endl;
            continue;
        }

        if( verbose ) cout << endl << "Best match: " << imax << " " << jmax << " " << Form("%.2f",maxpt) << endl;

        if (lep_pT->at(imax) < lep_pT->at(jmax)) {
            int temp = imax;
            imax = jmax;
            jmax = temp;
        }
