...
skipExisting    = False # will skip MC merging when *DSID* exists in the MC/ folder

useHaddTemplate = True # put template first
...
if doMC:
    ...
    for dsid in uniqIDs:
        ...
        if skipExisting and str(dsid) in commands.getoutput("ls " + putInDirMC):
            print "Skipping existing ntuple: " + str(dsid)
            continue
